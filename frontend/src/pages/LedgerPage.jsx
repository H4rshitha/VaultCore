...
export const LedgerPage = () => {
  ...
  const selectedAccount =
    accounts.find((a) => a.accountNumber === selectedAccountNumber) || accounts[0];

  // Optional real-time balance for the selected account
  const { data: balanceData } = useAccountBalance(selectedAccountNumber);
  const currentBalance =
    balanceData?.balance !== undefined ? balanceData.balance : selectedAccount?.balance;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800/80">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
            Immutable cryptographic journal entries with running balance verification and
            zero-discrepancy reconciliation.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
          ...
          <div className="md:col-span-2 space-y-2">
            ...
            {isAccountsLoading ? (
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                No active deposit accounts found.
              </p>
            ) : (
              <select
          ...
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 flex flex-col justify-center">
            ...
            <div className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white mt-1">
              {currentBalance !== undefined ? formatCurrency(currentBalance, currency) : '---'}
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-red-600 dark:text-red-400/80">
                {error?.response?.data?.message ||
                  error?.message ||
                  'Error communicating with Ledger Service'}
              </p>
            </div>
          </div>
          <button
