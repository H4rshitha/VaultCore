...
export const ProtectedRoute = ({ children, requiredRole }) => {
  ...
  if (requiredRole && currentUser?.role !== requiredRole && currentUser?.role !== USER_ROLES.ADMIN) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center text-center p-8">
        ...
        <p className="text-slate-400 max-w-md">
          Your account role (<span className="text-brand-400 font-semibold">{currentUser?.role || USER_ROLES.CUSTOMER}</span>) does not have permission to view this administrative resource.
        </p>
      </div>
    );
  }
