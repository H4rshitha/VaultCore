...
import { createLogger, InternalServerError, ledgerCircuitBreaker, CircuitBreakerOpenError } from '@vaultcore/shared';
  ...
  static async recordDoubleEntry({
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          lastError = new Error(data.error?.message || `Ledger service responded with status ${response.status}`);
          continue;
        }
        ...
        const errorMsg = isAbort ? `Ledger request timed out after ${timeoutMs}ms` : error.message;
    ...
    logger.error('All retry attempts to Ledger Service failed', {
      ...
      traceId,
        ...
        throw new InternalServerError(`Ledger service communication failure: ${lastError?.message || 'Unknown error'}`);
      },
      { traceId }
    );
  }
