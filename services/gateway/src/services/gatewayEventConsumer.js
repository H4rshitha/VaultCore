...
export class GatewayEventConsumer {
  ...
  async connectAndConsume() {
    ...
    try {
      if (this.logger) {
        this.logger.info(`[RabbitMQ] Connecting to broker at ${amqpUri.replace(/:[^:@]+@/, ':***@')}...`);
      }
      ...
      this.connection.on('close', () => {
            ...
            try {
              ...
              const sseEvent = {
                ...
                referenceId:
                  ...
                  headers.referenceId,
                amount:
                  eventPayload.amount !== undefined ? Number(eventPayload.amount) : undefined,
              ...
              if (routingKey === 'payment.completed' || eventType === 'PAYMENT_COMPLETED') {
                const amountFormatted = sseEvent.amount !== undefined ? `${sseEvent.amount} ${sseEvent.currency}` : 'funds';
        ...
        if (this.logger) {
          this.logger.info('[RabbitMQ] GatewayEventConsumer successfully started and listening for real-time events');
        }
      } catch (err) {
        if (this.logger) {
          this.logger.warn(
  ...
  scheduleReconnect() {
    ...
    if (this.logger) {
      this.logger.info(`[RabbitMQ] Scheduling reconnect attempt #${this.reconnectAttempts} in ${delay}ms`);
    }
  ...
  async stop() {
    ...
    try {
      ...
      if (this.logger) this.logger.warn(`[RabbitMQ] Error during clean consumer shutdown: ${err.message}`);
    } finally {
      this.cleanup();
