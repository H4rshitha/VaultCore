...
async function verify() {
  ...
  try {
    ...
    assert(
      ...
      `Signup endpoint returns 201 Created (got ${signupRes.status})`
    );
    assert(signupData.success === true, 'Signup response indicates success: true');
    assert(
      signupData.data?.user?.email === testEmail,
      `User created with email: ${signupData.data?.user?.email}`
    );

    // Check for Set-Cookie header
    const setCookie = signupRes.headers.get('set-cookie');
    assert(
      setCookie !== null && setCookie.includes('refreshToken='),
      `Set-Cookie contains refreshToken (${setCookie ? 'Found' : 'Missing'})`
    );
    assert(
      setCookie && setCookie.toLowerCase().includes('httponly'),
      'Refresh token cookie is flagged HttpOnly'
    );
  ...
  try {
    ...
    assert(
      ...
      'Gateway returned valid JWT access token'
    );

    const setCookie = loginRes.headers.get('set-cookie');
    assert(
      setCookie !== null && setCookie.includes('refreshToken='),
      'Login response sets HttpOnly refreshToken cookie'
    );
    if (setCookie) {
      receivedCookie = setCookie;
    }
  ...
  assert(
    !storageFile.includes("localStorage.setItem('vaultcore_access_token'") &&
      !storageFile.includes('localStorage.setItem("vaultcore_access_token"'),
    'Access token is NEVER persisted in localStorage'
  );
  assert(
    storageFile.includes('let inMemoryAccessToken = null;'),
    'Access token stored in memory variable'
  );
  assert(
    storageFile.includes('setMemoryAccessToken') && storageFile.includes('getMemoryAccessToken'),
    'In-memory token getter and setter exported'
  );
  ...
  assert(
    receivedCookie && receivedCookie.toLowerCase().includes('httponly'),
    'Cookie contains HttpOnly directive (inaccessible to JavaScript)'
  );
  assert(
    receivedCookie && receivedCookie.toLowerCase().includes('samesite=lax'),
    'Cookie contains SameSite=Lax directive for CSRF protection'
  );
  ...
  try {
    ...
    assert(
      typeof newAccessToken === 'string' && newAccessToken.length > 20,
      'Session restored with fresh JWT access token'
    );
  } catch (err) {
  ...
  assert(
    protectedRouteFile.includes('!isAuthenticated'),
    'ProtectedRoute checks isAuthenticated state'
  );
  assert(
    protectedRouteFile.includes('<Navigate to="/login"'),
    'ProtectedRoute redirects unauthenticated visitors to /login'
  );
  assert(
    protectedRouteFile.includes('state={{ from: location }}'),
    'ProtectedRoute preserves target location state for post-login redirect'
  );

  const appRoutesFile = fs.readFileSync('frontend/src/routes/AppRoutes.jsx', 'utf8');
  assert(
    appRoutesFile.includes('<ProtectedRoute>'),
    'Dashboard routes wrapped in ProtectedRoute guard'
  );
