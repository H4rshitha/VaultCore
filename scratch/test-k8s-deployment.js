...
const hasKubectl =
  /azure\/setup-kubectl@v\d+/i.test(content) ||
  /install\s+kubectl/i.test(content);
recordCheck(1, 'kubectl installation exists', hasKubectl, 'azure/setup-kubectl action step');

// 2. kustomize installation exists
const hasKustomize =
  /setup-kustomize@v\d+/i.test(content) ||
  /install\s+kustomize/i.test(content);
recordCheck(2, 'kustomize installation exists', hasKustomize, 'imranismail/setup-kustomize action step');
...
recordCheck(3, 'kubectl apply -k infrastructure/k8s exists', hasKubectlApplyKustomize, 'Applies manifests via Kustomize overlay');
...
recordCheck(4, 'Image replacement uses Kustomize', hasKustomizeSetImage, 'kustomize edit set image for all 6 microservices');
...
const hasReadinessVerification =
  ...
  (/kubectl\s+wait\s+--for=condition=ready\s+pod/i.test(content) || /readinessProbe/i.test(content) || /rollout status/i.test(content));
recordCheck(7, 'Readiness verification exists', hasReadinessVerification, 'kubectl wait for condition=ready across pods');

// 8. Rollout logs uploaded as artifacts
const hasLogsUpload =
  /actions\/upload-artifact@v\d+/i.test(content) &&
  /rollout-logs/i.test(content);
recordCheck(8, 'Rollout logs uploaded as artifacts', hasLogsUpload, 'actions/upload-artifact@v4 for k8s-rollout-logs');

// 9. $GITHUB_STEP_SUMMARY contains deployment status
const hasStepSummary =
  /\$GITHUB_STEP_SUMMARY/.test(content) &&
  /Kubernetes Deployment Summary/i.test(content);
recordCheck(9, '$GITHUB_STEP_SUMMARY contains deployment status', hasStepSummary, 'Step summary table with deployment and rollout status');

// 10. Workflow fails on rollout timeout
const failsOnTimeout =
  /--timeout=\d+s/i.test(content) &&
  !/continue-on-error:\s*true/i.test(content.slice(content.indexOf('Verify Deployment Rollouts'), content.indexOf('Verify Pod Readiness Probes')));
recordCheck(10, 'Workflow fails on rollout timeout', failsOnTimeout, 'Deterministic failure on timeout without ignoring exit code');

console.log('\n======================================================');
const allPassed = checks.every((c) => c.pass);
if (allPassed) {
  console.log(`🎉 All ${checks.length}/${checks.length} Kubernetes Deployment Pipeline checks PASSED!`);
  process.exit(0);
} else {
