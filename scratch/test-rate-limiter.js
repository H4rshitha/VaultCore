...
async function runTests() {
  ...
  assert.strictEqual(
    RATE_LIMIT_CONFIG.PAYMENT.SUSTAINED_LIMIT,
    20,
    'Payment sustained limit must be 20 req/min'
  );
  assert.strictEqual(RATE_LIMIT_CONFIG.PAYMENT.BURST, 5, 'Payment burst allowance must be 5 req');
  assert.strictEqual(
    RATE_LIMIT_CONFIG.PAYMENT.LIMIT,
    25,
    'Payment total limit with burst must be 25 req/min'
  );
  assert.strictEqual(RATE_LIMIT_CONFIG.GENERAL.LIMIT, 100, 'General limit must be 100 req/min');
  console.log(
    '  ✔ Redis DB0 allocated; Payment Tier configured with 20 sustained + 5 burst = 25 limit'
  );
  ...
  assert.strictEqual(
    getClientIp(reqA),
    '198.51.100.1',
    'Priority 1: req.ip is chosen when present'
  );
  ...
  assert.strictEqual(
    getClientIp(reqB),
    '203.0.113.50',
    'Priority 2: First IP from X-Forwarded-For is chosen'
  );
  ...
  assert.strictEqual(
    getClientIp(reqC),
    '192.168.1.100',
    'Priority 3: req.socket.remoteAddress is chosen'
  );
  ...
  assert.strictEqual(
    isExcludedRoute({ path: '/docs/swagger-ui.css' }),
    true,
    '/docs/* assets are excluded'
  );
  assert.strictEqual(
    isExcludedRoute({ path: '/api/v1/payments/transfer' }),
    false,
    'Payment API is not excluded'
  );
  ...
  console.log(
    '  ✔ Sliding window allows up to quota, accurately counts down, and returns integer retryAfter'
  );
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      assert.strictEqual(
        hRes.headers.get('x-ratelimit-limit'),
        null,
        '/health does not set rate limit headers'
      );

      const mRes = await fetch(`${baseUrl}/metrics`);
      assert.strictEqual(mRes.status, 200);
      assert.strictEqual(
        mRes.headers.get('x-ratelimit-limit'),
        null,
        '/metrics does not set rate limit headers'
      );
    }
    ...
    for (let i = 1; i <= 11; i++) {
      ...
      if (res.status === 429) {
        ...
        assert.strictEqual(
          Number.isInteger(Number(authHeaders.retryAfter)),
          true,
          'Retry-After must be an integer'
        );
        assert.strictEqual(
          Number.isInteger(Number(authHeaders.reset)),
          true,
          'X-RateLimit-Reset must be an integer timestamp'
        );
        ...
        console.log(
          `    ✔ Request #${i} blocked with HTTP 429; Retry-After: ${authHeaders.retryAfter}s (integer verified)`
        );
        break;
      }
    }
    ...
    for (let i = 1; i <= 26; i++) {
      ...
      if (res.status !== 429) {
        assert.strictEqual(
          limitHeader,
          '25',
          'Payment limit header reflects 25 (20 sustained + 5 burst)'
        );
        assert.strictEqual(
          Number(remainingHeader),
          25 - i,
          `Remaining quota correctly decrements to ${25 - i}`
        );
        ...
        assert.strictEqual(
          ...
          true,
          'Retry-After on payment 429 is integer seconds'
        );
        console.log(
          `    ✔ Successfully permitted 25 burst requests; blocked request #26 with HTTP 429 (Retry-After: ${retryAfter}s)`
        );
        break;
      }
    }
    ...
    assert.strictEqual(
      paymentRequestsCompleted,
      25,
      'Exact 25 requests permitted before throttling'
    );
    ...
    assert.strictEqual(
      resGeneral.headers.get('x-ratelimit-limit'),
      '100',
      'General endpoint quota is 100 req/min'
    );
